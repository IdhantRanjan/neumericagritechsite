...
3. **Buy the sensor, own the interpretation.** No hardware. Three sensor tiers feed one FCR schema — **satellite** (Sentinel-2, always-on monitoring/triage at 10 m), **drone** (farmer-operated, 1–3 cm, claim-scale quantification), **phone** (ground corroboration + labels). Which tier answers a given question is decided by an explicit, versioned, **logged routing rule** (`sensor-routing@1.0.0`), not an ML guess — see ENGINES.md §0. Neumeric's asset is the interpretation layer and the accumulated labeled record base, not pixels.
